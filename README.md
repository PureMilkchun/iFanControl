...
iFanControl 会默认发送每天最多一次的匿名活跃统计，帮助个人开发者判断软件是否仍被真实使用。统计内容仅包含随机匿名 ID、版本号和 build；不会发送姓名、邮箱、序列号、设备名称或风扇/温度数据。你可以在 `关于/帮助 -> 简介` 中关闭“共享匿名活跃统计”。
